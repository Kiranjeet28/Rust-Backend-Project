...
use crate::models::request::add_card_request::AddCardRequest;
...
impl MongoDB {
            ...
            collection
                .find_one_and_replace(
                    ...
                    User {
                        ...
                        card_items: edit_model.card_items.clone()
                    },
                    None
                )
                .await?
    ...
    pub async fn add_card(
        ...
        let existing_user = collection
            .find_one(bson::doc! { "mail": &email }, None)
            .await?
            .expect("User not found");

        // Combine existing card items with new card items
        let mut updated_card_items = existing_user.card_items.unwrap_or_default();
        if let Some(new_items) = &edit_model.card_items {
            updated_card_items.extend(new_items.clone());
        }

        // Ensure card_items is a unique set to avoid duplicates
        updated_card_items.sort();
        updated_card_items.dedup();

        // Update the user's card items
        collection
            .update_one(
                bson::doc! { "mail": &email },
                bson::doc! { "$set": { "card_items": updated_card_items } },
                None,
            )
            .await?;
            ...
            FindUserBy::UserNotFound => match hash_text(registration_request.password.clone(), 4) {
                Ok(hash_password) => {
                    let user = User {
                        ...
                        card_items: registration_request.card_items.clone()
                    };
